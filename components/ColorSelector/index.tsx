...
const ColorSelector: FC<ColorSelectorProps> = ({
  ...
  const { textId, slideId, style, background } =
    useEditor()
  ...
  const [mode, setMode] =
    useState<'gradient' | 'mono' | 'image'>('gradient')
  ...
  return (
    <>
      {isBackground && (
        <View style={[ColorSelectorStyle.modeContainer]}>
          {BACKGROUND_TYPE.map((t) => {
            const backType = t.toLowerCase() as typeof mode
            return (
              <View key={t} style={ColorSelectorStyle.mode}>
                <Text>{t}</Text>
                <Switch
                  value={mode === backType}
                  onValueChange={(v) =>
                    onChangeMode(v, backType)
                  }
                />
              </View>
            )
      ...
      <View style={ColorSelectorStyle.container}>
        ...
        {(!isImage && !isGradient) ||
          (isText && (
            <View style={ColorSelectorStyle.colorContainer}>
              {COLORS.map((c) => (
                <Color
                  key={c}
                  active={c === color}
