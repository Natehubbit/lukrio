...
const Editor: FC<EditorProps> = () => {
  const { slideId, textType, textId, hasText } = useEditor()
    ...
    <MotiView
      ...
      {<View>
        {Object.keys(EDIT_TOOLS).map((k, i) => {
          ...
          const textExpanded = textType === 'text' && textId !== undefined
          const imageExpanded = textType === 'image' && textId !== undefined
          ...
          return <EditorItem
            ...
            {(hasText && isText)
              ? <>
                ...
                {isText && <FontSelector />}
                {isText && <ColorSelector type='text' />}
              </>
              : isBack
                ? <ColorSelector isGradient type='background' />
                : isAddImg
                  ? <ImageSelector onSelect={onAddImage} />
                  : !isIcon && <View style={[EditorStyle.noTextSelected]}>
                    <Text>
